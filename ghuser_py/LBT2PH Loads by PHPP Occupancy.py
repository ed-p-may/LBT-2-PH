...
#
"""
Use this component to set the 'occupancy' of the building for the PHPP. Be sure 
to use this component BEFORE using the 'PHPP Res. Appliances' component since the 
total occupancy is needed in order to estimate the yearly applince energy consumption.
-
Note that by default this component will ONLY set the PHPP values, not the Honeybee 
load and schedue. If  you would like to set the Honeybee load / schedule to match 
the PHPP, set the 'set_honeybee_loads_' value to TRUE. This will set the Honyebee 
occupancy schedule to a 'Constant' schdedule in order to try and approximate the 
PHPP as closely as possible.
-
EM March 1, 2021
    Args:
        _HB_model: The Honeybee Model
        set_honeybee_loads_: (bool) Default=False. Set this to TRUE if you would 
            like to set the Honeybee occupant load / schedule to match the PHPP values.
        num_res_units_: (int) Default=1. The total number of residential 'units' in 
            the model. For single-family=1, for multi-family enter the numer of 
            individual dwelling units. For ALL non-residential, leave blank or enter '1'.
        occupancy_: (int) Default=Automatic. The annual average number of occupants. 
            For All residential, leave this blank and the PHPP will calculate this 
            value automatically. Only set it in very unique circumstances. For ALL 
            non-residential, enter the value here.
        
        buildingType_: <Optional> Input either: "1-Residential building" or 
            "2-Non-residential building"
        ihgType_: <Optional> Internal Heat Gains Type. Input either: "10-Dwelling", 
            "11-Nursing home / students", "12-Other", "20-Office / Admin. building", 
            "21-School", or "22-Other"
        ihgValues_: <Optional> Internal Heat Gains Source. 
            > For Residential, Input either: "2-Standard" or "3-PHPP calculation 
            ('IHG' worksheet)"
            > For Non-Residential, input either: "2-Standard" or "4-PHPP calculation 
            ('IHG non-res' worksheet)"
    Returns:
...
ghenv.Component.Name = "LBT2PH Loads by PHPP Occupancy"

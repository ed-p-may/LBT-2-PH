...
def preview_obj(_classObj):
    ...
    if not hasattr(_classObj, '__dict__'):
        print('{} object "{}" has no "__dict__" attribute.'.format(
            type(_classObj), _classObj))
        return None
        ...
        try:
            ...
            if string_found or value_found:
                ...
                try:
                    ...
                    print('Converting input "{}" >>> {} * {} = {} {}'.format(inputValue,
                          value_found, conversion_factor, output_val, _outputUnit))
                except ValueError as e:
                    output_val = float(
                        eval('({}{})'.format(value_found, conversion_factor)))
                    print('Converting input "{}" >>> ({}{}) = {} {}'.format(
                        inputValue, value_found, conversion_factor, output_val, _outputUnit))
...
def get_rh_obj_UserText_dict(_ghdoc, _rh_obj_guid):
    ...
    with context_rh_doc(_ghdoc):
        ...
        object_rh_UserText_dict = {k: rs.GetUserText(
            rh_obj, k) for k in rs.GetUserText(rh_obj)}
